...
const UsersElectricityByDate = ({ startDate, endDate }) => {
  ...
  const fetchElectricityUsageUsers = async () => {
    try {
      const response = await api.get(`/api/electricity-usage/${userId}/range`, {
        params: {
          startDate,
          endDate
        },
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold mb-2">Dữ liệu cập nhật từ ngày {startDate ? new Date(startDate).toLocaleDateString() : 'N/A'} đến ngày {endDate ? new Date(endDate).toLocaleDateString() : 'N/A'}</h2>
...
export default UsersElectricityByDate;
